...
function PerimeterMarquee({ color = '#ffffff' }) {
  ...
  useEffect(() => {
    ...
    const launch = () => {
      // measure with standalone <text>, not textPath (textPath clips to path length)
      if (measureRef.current) {
        try {
          const l = measureRef.current.getComputedTextLength();
          if (l > 0) unitPx = l;
        } catch {}
      }
    ...
    // wait for fonts to load before measuring — avoids wrong fallback metrics
  ...
  return (
    <svg className="nf-perimeter-svg" viewBox={`0 0 ${w} ${h}`} aria-hidden>
      <defs>
...
export default function NotFound() {
  ...
  const drawWheelPixels = useCallback(() => {
    ...
    const SIZE = cvs.parentElement?.clientWidth || 290;
