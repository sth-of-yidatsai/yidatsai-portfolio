...
vite-plugin-prerender → build 時預渲染所有頁面為靜態 HTML
public/sitemap.xml  → 自動產生，告知 Google 所有頁面
```

### 運作流程

1. 瀏覽器首次載入 → 讀取 `index.html` 的靜態 meta（社群分享時爬蟲看到的）
2. React 掛載後 → `useMeta()` 依路由動態覆蓋 `<head>` 內容
3. `npm run build` 時 → `vite-plugin-prerender` 用 Puppeteer 預渲染每頁，產出靜態 HTML（Google 爬蟲可直接讀取，不需執行 JS）
...
1. **`index.html`** — JSON-LD Person schema 的 `alternateName` 與 `description` 欄位
...
`renderAfterTime: 3000` — 每頁等待 3 秒讓 React 渲染完成，共 N 頁 × 3 秒會增加 build 時間。
...
public/images/projects/my-new-project/
  ...
  ├── 02.webp
  ├── ...
  └── og.jpg        ← OG 圖（選填，1200×630px）
```

響應式圖片可用 `npm run generate-images` 批次產生（需先確認腳本設定）。
...
`public/sitemap.xml` 在每次 `npm run build` 前由 `scripts/generate-sitemap.js` 自動產生，無需手動維護。

新作品加入 `projects.json` 後，下次 build sitemap 會自動更新。
...
2. 提交 `https://yidatsai.com/sitemap.xml`
3. 新作品頁面用「URL 檢查」→「要求建立索引」加速收錄
